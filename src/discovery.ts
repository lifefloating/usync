...
import { exists, walkFiles } from "./file-utils.js";
import { toTemplatePath } from "./path-template.js";
...
export async function discoverSyncItems(
  ...
  for (const provider of providers) {
    ...
    for (const target of targets) {
      if (!(await exists(target.absolutePath))) continue;

      if (!target.isDirectory) {
        if (shouldExclude(target.absolutePath)) continue;
      ...
      for (const filePath of files) {
        ...
        const rel = path.relative(target.absolutePath, filePath);
        items.push({
          provider: provider.name,
          category: target.category,
