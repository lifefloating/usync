...
import { GistClient, type GistResponse } from "./gist-client.js";
import { resolveTemplatePath } from "./path-template.js";
...
function templatePathToVisibleOutputPath(templatePath: string, provider: ManifestItem["provider"]): string {
  ...
  const scope = normalized.startsWith("$HOME/")
    ? "home"
    : normalized.startsWith("$PROJECT/")
      ? "project"
      : "absolute";

  const withoutToken = normalized
    .replace(/^\$HOME\//, "")
    .replace(/^\$PROJECT\//, "")
    .replace(/^\//, "");
  ...
  return path.join(scope, providerAlias, relative);
}
...
export function parseManifestFromGist(gist: GistResponse): SyncManifest | null {
  ...
  try {
    ...
    if (parsed.version !== 1 || !parsed.items) return null;
    return parsed;
  } catch {
    return null;
  }
...
export function buildUploadPlan(localItems: SyncItem[], remoteManifest: SyncManifest | null): UploadPlan {
  ...
  for (const item of localItems) {
    ...
    if (!remote || remote.hash !== localHash) changed.push(item);
...
export async function uploadToGist(input: {
  ...
  for (const templatePath of plan.deletedTemplatePaths) {
    ...
    if (oldGistFile) filesPatch[oldGistFile] = null;
    progress += 1;
...
export async function downloadFromGist(input: {
  ...
  if (!manifest) {
    ...
    for (const [fileName, gistFile] of entries) {
      ...
      const targetPath = path.join(path.resolve(input.outputRoot), "raw-gist", fileName);
